...
folder_paths.folder_names_and_paths["instantid"] = (
    current_paths,
...
def _set_model_patch_replace(model, patch_kwargs, key):
    ...
    if key not in to["patches_replace"]["attn2"]:
        to["patches_replace"]["attn2"][key] = Attn2Replace(
            instantid_attention, **patch_kwargs
        )
...
class FaceKeypointsPreprocessor:
    ...
    def preprocess_image(self, faceanalysis, image):
        ...
        if face_kps is None:
            ...
            print(
                f"\033[33mWARNING: no face detected, unable to extract the keypoints!\033[0m"
            )
    ...
    def apply_instantid(
        ...
        if clip_embed.shape[0] > 1:
            if combine_embeds == "average":
                ...
                clip_embed = torch.mean(
                    clip_embed / torch.norm(clip_embed, dim=0, keepdim=True), dim=0
                ).unsqueeze(0)
        ...
        image_prompt_embeds, uncond_image_prompt_embeds = (
            self.instantid.get_image_embeds(
        ...
        for id in range(6):  # id of output_blocks that have cross attention
            ...
            for index in block_indices:
                patch_kwargs["module_key"] = str(number * 2 + 1)
                _set_model_patch_replace(
                    work_model, patch_kwargs, ("output", id, index)
                )
                number += 1
            ...
            for t in conditioning:
                ...
                if prev_cnet in cnets:
                    ...
                    c_net = control_net.copy().set_cond_hint(
                        face_kps.movedim(-1, 1), cn_strength, (start_at, end_at)
                    )
...
class InstantIDAttentionPatch:
    ...
    def patch_attention(
        ...
        if weight == 0:
            return (
                model,
                {"cond": image_prompt_embeds, "uncond": uncond_image_prompt_embeds},
            )
        ...
        for conditioning in [positive, negative]:
            ...
            for t in conditioning:
                ...
                if prev_cnet in cnets:
                    ...
                    c_net = control_net.copy().set_cond_hint(
                        control_hint, strength, (start_at, end_at)
                    )
                    c_net.set_previous_controlnet(prev_cnet)
                    cnets[prev_cnet] = c_net
                ...
                d["control_apply_to_uncond"] = False
                d["cross_attn_controlnet"] = (
                    image_prompt_embeds.to(comfy.model_management.intermediate_device())
                    if is_cond
                    else uncond_image_prompt_embeds.to(
                        comfy.model_management.intermediate_device()
                    )
